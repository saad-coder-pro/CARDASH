/* ============================================================
   ...
   Load with:  <script defer src="keyboard.js"></script>
   Requires (loaded via defer in head):
     - SimpleKeyboard
     - GSAP  (optional; falls back gracefully)
   ============================================================ */
(function () {
    "use strict";

    /* ---------- STATE ---------- */
    var MIN_H = 28, MAX_H = 100;
    ...
    /* System keyboard proxy mode — shared across closures */
    var systemKbActive = false;      // sysmode is on
    var sysmodeTarget = null;        // main field we're proxying for
    var suppressRefocus = false;     // suppress refocus during transitions
    ...
    function injectCSS() {
        ...
        var css =
            ':root{--vk-accent:#0a84ff;--vk-text:#fff;' +
            '--vk-key:#050505;--vk-key-hover:#1a1a1a;--vk-key-alt:#131316;--vk-key-alt-hover:#232326;--vk-key-text:#fff;' +
            ...
            '.kb-wrap{position:fixed;left:0;right:0;bottom:0;width:100%;' +
            'max-height:65vh;max-height:65dvh;' +
            'z-index:2147483000;padding-bottom:env(safe-area-inset-bottom);' +
            ...
            'box-shadow:0 -20px 60px rgba(0,0,0,.65),0 -2px 12px rgba(0,0,0,.45),inset 0 1px 0 rgba(255,255,255,.30);' +
            'transform:translateY(100%);visibility:hidden;' +
            'will-change:transform;-webkit-touch-callout:none;-webkit-user-select:none;user-select:none;' +
            ...
            '.kb-wrap.open{transform:translateY(0);visibility:visible;transition:transform .32s cubic-bezier(.4,0,.2,1),visibility 0s;}' +
            ...
            '.kb-icon-btn{width:38px;height:38px;display:flex;align-items:center;justify-content:center;' +
            'border:1px solid rgba(255,255,255,.16);border-radius:50%;background:rgba(10,10,12,.55);color:var(--vk-text);' +
            'font-size:17px;cursor:pointer;box-shadow:inset 0 1px 0 rgba(255,255,255,.18),0 4px 14px rgba(0,0,0,.6);' +
            ...
            /* ================= SYSTEM KEYBOARD PROXY FIELD ================= */
            '.kb-sysinput-wrap{display:none;flex-direction:column;gap:14px;padding:10px 18px 22px;flex:1 1 auto;' +
            'min-height:150px;justify-content:center;position:relative;z-index:5}' +
            '.kb-wrap.sysmode .kb-sysinput-wrap{display:flex}' +
            '.kb-wrap.sysmode .simple-keyboard{display:none}' +
            '.kb-wrap.sysmode .kb-resize-handle{display:none}' +

            '.kb-sysinput-label{display:flex;align-items:center;gap:8px;font-size:11px;font-weight:900;' +
            'letter-spacing:.5px;text-transform:uppercase;color:rgba(255,255,255,.55);padding:0 6px;' +
            'overflow:hidden;text-overflow:ellipsis;white-space:nowrap}' +
            '.kb-sysinput-label > i{color:#ffb340;font-size:12px;flex-shrink:0}' +
            '.kb-sysinput-label > span{flex-shrink:0}' +
            '.kb-sysinput-target{color:#fff;text-transform:none;letter-spacing:-.005em;font-weight:900;font-size:12.5px;' +
            'max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;' +
            'padding:3px 9px;border-radius:999px;background:rgba(255,159,10,.16);border:1px solid rgba(255,159,10,.3)}' +

            '.kb-sysinput{width:100%;padding:15px 18px;border-radius:14px;border:1px solid rgba(255,159,10,.45);' +
            'background:rgba(255,255,255,.08);color:#fff;font-family:var(--vk-font);font-size:15px;font-weight:700;' +
            'outline:none;caret-color:#ff9f0a;transition:border-color .18s ease,background .18s ease,box-shadow .18s ease;' +
            '-webkit-appearance:none;appearance:none;' +
            'box-shadow:0 0 0 3px rgba(255,159,10,.12)}' +
            '.kb-sysinput:focus{border-color:rgba(255,159,10,.85);background:rgba(255,255,255,.12);' +
            'box-shadow:0 0 0 3px rgba(255,159,10,.28),0 0 22px rgba(255,159,10,.25)}' +
            '.kb-sysinput::placeholder{color:rgba(255,255,255,.28);font-weight:600}' +
            /* ================= END SYSTEM KEYBOARD PROXY FIELD ================= */
            ...
            'transform:translateY(102%);opacity:0;pointer-events:none;will-change:transform,opacity;' +
            'transition:transform .3s cubic-bezier(.34,1.15,.64,1),opacity .2s ease;color:var(--vk-text);font-family:var(--vk-font)}' +
            ...
            '.kb-settings-overlay__back{appearance:none;width:36px;height:36px;display:flex;align-items:center;justify-content:center;' +
            'border-radius:50%;border:1px solid rgba(255,255,255,.14);background:rgba(0,0,0,.35);color:#fff;cursor:pointer;padding:0}' +
            '.kb-settings-overlay__back:hover{background:rgba(255,255,255,.10)}' +
            '.kb-settings-overlay__title{font-size:15px;font-weight:900;color:#fff}' +
            '.kb-settings-overlay__spacer{width:36px;height:36px}' +
            '.kb-settings-overlay__body{flex:1 1 auto;min-height:0;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;padding:10px 16px 24px;width:100%}' +
            ...
            'button.kb-settings-row{appearance:none;width:100%;border:none;background:transparent;color:#fff;font-family:var(--vk-font);text-align:left;cursor:pointer;padding:12px 14px}' +
            ...
            '.kb-settings-row__chev{color:rgba(255,255,255,.32);flex-shrink:0}' +
            ...
            '.kb-color-picker{position:relative;width:34px;height:34px;border-radius:50%;overflow:hidden;border:2px solid rgba(255,255,255,.22);cursor:pointer;flex-shrink:0}' +
            ...
            '.kb-color-presets{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}' +
            '.kb-color-preset{width:24px;height:24px;border-radius:50%;border:2px solid rgba(255,255,255,.12);cursor:pointer;padding:0;background:var(--swatch);position:relative;touch-action:manipulation}' +
            '.kb-color-preset.active{border-color:#0a84ff;box-shadow:0 0 0 2px rgba(10,132,255,.35),0 0 12px rgba(10,132,255,.55)}' +
            '.kb-color-preset.active::after{content:"";position:absolute;inset:0;margin:auto;width:5px;height:5px;border-radius:50%;background:#fff}' +
            ...
            '.kb-seg__btn{appearance:none;border:none;background:transparent;color:rgba(255,255,255,.65);font-family:var(--vk-font);font-weight:900;font-size:12px;padding:6px 12px;border-radius:8px;cursor:pointer;display:flex;align-items:center;gap:5px}' +
            ...
            '.kb-resize-handle__grip{width:100%;height:4px;border-radius:3px;background:linear-gradient(90deg,rgba(10,132,255,0) 0%,#0a84ff 15%,#6cb8ff 50%,#0a84ff 85%,rgba(10,132,255,0) 100%);box-shadow:0 0 8px rgba(10,132,255,.9),0 0 22px rgba(10,132,255,.55);pointer-events:none}' +
            '.kb-resize-handle.dragging .kb-resize-handle__grip{height:6px;box-shadow:0 0 14px rgba(10,132,255,1),0 0 34px rgba(10,132,255,.85)}' +
            '.kb-resize-toast{position:fixed;top:max(18px,env(safe-area-inset-top));left:50%;transform:translateX(-50%) translateY(-16px);padding:9px 16px;border-radius:999px;background:rgba(20,20,26,.94);border:1px solid rgba(255,255,255,.14);color:#fff;font-size:12px;font-weight:800;box-shadow:0 10px 28px rgba(0,0,0,.65);z-index:2147483002;opacity:0;pointer-events:none;transition:opacity .22s ease,transform .28s cubic-bezier(.34,1.4,.64,1);display:flex;align-items:center;gap:8px;white-space:nowrap;font-family:var(--vk-font)}' +
            ...
            '.kb-dark.hg-theme-default .hg-button{position:relative;height:var(--vk-key-h);background:var(--vk-key);color:var(--vk-key-text);' +
            'border:1px solid rgba(255,255,255,.10);border-radius:var(--vk-key-radius);font-family:var(--vk-font);font-size:var(--vk-key-fs);font-weight:900;' +
            'display:flex;align-items:center;justify-content:center;padding:0;overflow:hidden;touch-action:manipulation;user-select:none;-webkit-user-select:none;-webkit-touch-callout:none;' +
            'box-shadow:var(--vk-key-shadow);transition:background .12s ease,border-color .12s ease,color .12s ease;flex-grow:1;flex-basis:0;min-width:0}' +
            '.kb-dark.hg-theme-default .hg-button:hover{background:var(--vk-key-hover);border-color:rgba(255,255,255,.18)}' +
            '.kb-dark.hg-theme-default .hg-button:active,.kb-dark.hg-theme-default .hg-button.hg-activeButton{background:var(--vk-accent);border-color:rgba(255,255,255,.35);color:#fff;' +
            'box-shadow:0 0 0 1px rgba(10,132,255,.5),0 0 24px rgba(10,132,255,.55),inset 0 1px 0 rgba(255,255,255,.35)}' +
            ...
            '.kb-dark.hg-theme-default .hg-button.hg-button-123,.kb-dark.hg-theme-default .hg-button.hg-button-abc{font-size:calc(var(--vk-key-fs)*.85);font-weight:900}' +
            ...
            '.kb-dark.hg-theme-default .hg-button.key-toggle-active{filter:brightness(1.2);border-color:rgba(10,132,255,.55) !important}' +
            '.kb-dark.hg-theme-default .hg-button.key-toggle-active::after{content:"";position:absolute;top:6px;right:6px;width:7px;height:7px;border-radius:50%;background:#0a84ff;box-shadow:0 0 6px rgba(10,132,255,.95),0 0 14px rgba(10,132,255,.65);pointer-events:none}' +
            ...
            '@media (max-width:480px){:root{--vk-key-h:42px;--vk-pad-x:8px;--vk-pad-y:12px}.kb-wrap{border-top-left-radius:24px;border-top-right-radius:24px}.kb-sysinput-wrap{padding:10px 14px 20px;min-height:140px}}' +
            ...
            '@media (orientation:landscape) and (max-height:500px){:root{--vk-key-h:34px;--vk-pad-x:16px;--vk-pad-y:8px}.kb-wrap{max-height:80vh;border-top-left-radius:18px;border-top-right-radius:18px}.kb-sysinput-wrap{min-height:110px}}';
    ...
    function injectHTML() {
        ...
        wrap.innerHTML =
            '<div class="kb-wrap" id="kbWrap">' +
              '<div class="kb-header">' +
                '<button class="kb-icon-btn kb-system-kb-btn" id="kbSystemKbBtn" type="button" aria-label="Use system keyboard" title="Use system keyboard">' +
                ...
                '<button class="kb-icon-btn kb-settings-btn" id="kbSettingsBtn" type="button" aria-label="Keyboard settings">' +
                  ...
                  '</svg>' +
                '</button>' +
                '<button class="kb-icon-btn kb-close" id="kbClose" type="button" aria-label="Close keyboard">' +
              ...
              /* System keyboard proxy area */
              '<div class="kb-sysinput-wrap" id="kbSysInputWrap">' +
                '<div class="kb-sysinput-label">' +
                  '<i class="fa-solid fa-keyboard"></i>' +
                  '<span>Typing for</span>' +
                  '<span class="kb-sysinput-target" id="kbSysInputTarget">Field</span>' +
                '</div>' +
                '<input type="text" class="kb-sysinput" id="kbSysInput" autocomplete="off" autocorrect="on" autocapitalize="sentences" spellcheck="true" placeholder="Type here with your system keyboard...">' +
              '</div>' +
              ...
              '<div class="kb-settings-overlay" id="kbSettingsOverlay" role="dialog" aria-label="Keyboard settings">' +
                '<div class="kb-settings-overlay__nav">' +
                  '<button class="kb-settings-overlay__back" id="kbSettingsBack" type="button" aria-label="Back">' +
                  ...
                  '<div class="kb-settings-group">' +
                    '<div class="kb-settings-row"><span class="kb-settings-row__label">Icon Pack</span>' +
                      '<div class="kb-seg" id="iconPackSeg">' +
                        '<button type="button" data-pack="fa" class="kb-seg__btn"><i class="fa-brands fa-font-awesome"></i> FA</button>' +
                        '<button type="button" data-pack="ri" class="kb-seg__btn"><i class="ri-remixicon-line"></i> RI</button>' +
                      '</div>' +
                    '</div>' +
                    '<div class="kb-settings-row kb-settings-row--stack">' +
                      '<div class="kb-settings-row__top"><span class="kb-settings-row__label">Key Color</span>' +
                        '<label class="kb-color-picker"><input type="color" id="keyColorPicker" value="#050505"><span class="kb-color-picker__swatch" id="keyColorSwatch"></span></label>' +
                      '</div>' +
                      '<div class="kb-color-presets" id="keyColorPresets"></div>' +
                    '</div>' +
                    '<div class="kb-settings-row kb-settings-row--stack">' +
                      '<div class="kb-settings-row__top"><span class="kb-settings-row__label">Keyboard Color</span>' +
                        '<label class="kb-color-picker"><input type="color" id="kbColorPicker" value="#a0a0aa"><span class="kb-color-picker__swatch" id="kbColorSwatch"></span></label>' +
                      '</div>' +
                      '<div class="kb-color-presets" id="kbColorPresets"></div>' +
                    '</div>' +
                  '</div>' +
                  ...
                  '<div class="kb-settings-group">' +
                    '<button type="button" class="kb-settings-row" id="kbAdjustHeightBtn">' +
                      ...
                      '<span class="kb-settings-row__value"><span id="kbHeightValue">48px</span><span class="kb-auto-badge" id="kbAutoBadge" hidden>AUTO</span>' +
                        '<svg class="kb-settings-row__chev" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.8" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 6 15 12 9 18"></polyline></svg>' +
                      '</span>' +
                    '</button>' +
              ...
              '<div class="kb-resize-handle" id="kbResizeHandle" role="slider" aria-label="Resize keyboard"><div class="kb-resize-handle__grip"></div></div>' +
    ...
    /* ============================================================
       3. INPUTMODE=NONE ON ALL TEXT INPUTS (except our own)
       ============================================================ */
    var EDITABLE_SELECTOR = 'input:not([type]),input[type="text"],input[type="url"],input[type="search"],input[type="email"],input[type="tel"],input[type="password"],textarea,[contenteditable="true"]';
    ...
    function setupInputModeNone() {
        ...
        if (isIOS) {
            document.addEventListener("focusin", function (e) {
                ...
                if (isOwnInput(el)) return;   // never apply readOnly trick to our proxy
    ...
    function init() {
        ...
        if (!KeyboardCtor) {
            ...
            var t = setInterval(function () {
                ...
                KeyboardCtor = (typeof SK === "function") ? SK : (SK && SK.default);
                if (KeyboardCtor || ++tries > 100) {
                    clearInterval(t);
                    if (KeyboardCtor) start(KeyboardCtor);
                    else console.error("[vk] SimpleKeyboard never loaded");
                }
            }, 30);
            return;
        }
    ...
    function start(KeyboardCtor) {
        ...
        var kbSysInputWrap = document.getElementById("kbSysInputWrap");
        var kbSysInput = document.getElementById("kbSysInput");
        var kbSysInputTarget = document.getElementById("kbSysInputTarget");
        ...
        /* Enter sysmode: hide keys, show proxy text field, focus it */
        function enterSystemMode() {
            if (!currentInput) return;

            sysmodeTarget = currentInput;
            systemKbActive = true;

            // Fill proxy with current value
            var v = getVal(sysmodeTarget);
            kbSysInput.value = v;

            // Update label
            kbSysInputTarget.textContent = labelForInput(sysmodeTarget);

            // Apply class → shows proxy, hides keys
            kbWrap.classList.add("sysmode");
            updateSystemKbBtnState();

            // Blur the main input so iOS/Android don't keep the native keyboard on it
            try { sysmodeTarget.blur(); } catch(e){}

            // Focus the proxy input after a short delay so blur settles
            setTimeout(function(){
                try {
                    kbSysInput.focus();
                    // Place caret at end
                    var len = kbSysInput.value.length;
                    try { kbSysInput.setSelectionRange(len, len); } catch(e){}
                } catch(e){}
            }, 60);
        }

        /* Exit sysmode: hide proxy, restore keys, refocus target */
        function exitSystemMode(opts) {
            opts = opts || {};
            if (!systemKbActive && !kbWrap.classList.contains("sysmode")) return;

            var prevTarget = sysmodeTarget;

            systemKbActive = false;
            sysmodeTarget = null;

            kbWrap.classList.remove("sysmode");
            updateSystemKbBtnState();

            // Blur the proxy
            try { kbSysInput.blur(); } catch(e){}

            // If asked to restore virtual keyboard on the target
            if (opts.refocus && prevTarget && isEditableEl(prevTarget)) {
                currentInput = prevTarget;
                keyboard.setInput(getVal(prevTarget));
                showKeyboard();
                setTimeout(function(){
                    try { prevTarget.focus({ preventScroll: true }); } catch(e){}
                }, 50);
            }
        }

        /* -------- Proxy input: mirror typing to the target field -------- */
        kbSysInput.addEventListener("input", function(){
            if (!sysmodeTarget) return;
            var text = kbSysInput.value;
            setVal(sysmodeTarget, text);
            // Fire an input event on the target so any listeners know
            try {
                var ev = new Event("input", { bubbles: true });
                sysmodeTarget.dispatchEvent(ev);
            } catch(e){}
        });

        /* Keep caret position on target roughly in sync (optional nicety) */
        kbSysInput.addEventListener("select", function(){
            if (!sysmodeTarget) return;
            try {
                var s = kbSysInput.selectionStart;
                sysmodeTarget.setSelectionRange(s, s);
            } catch(e){}
        });

        /* Toggle button: enter/exit sysmode */
        kbSystemKbBtn.addEventListener("pointerdown", function(e){
            // Don't steal focus from the proxy while typing
            e.preventDefault();
        });
        kbSystemKbBtn.addEventListener("click", function(e){
            ...
            if (systemKbActive) {
                // toggle off → back to virtual keyboard
                exitSystemMode({ refocus: true });
            } else {
                // toggle on → system keyboard proxy
                if (!currentInput) {
                    // No focused field — pick the proxy itself
                    return;
                }
                enterSystemMode();
            }
        ...
        kbRoot.addEventListener("pointerdown", function (e) {
            ...
            if (hasGSAP) {
                ...
                gsap.timeline().set(btn, { scale: .85 })
                    .to(btn, { scale: 1, duration: .35, ease: "elastic.out(1, 0.45)" });
            }
        ...
        function openSettings() {
            // Don't allow settings while in sysmode
            if (systemKbActive) return;
        ...
        function computeAutoHeight() {
            ...
            var ratio = isLandscape ? (isSmall ? 0.55 : 0.42) : (isSmall ? 0.36 : 0.28);
        ...
        kbSettingsBtn.addEventListener("click", function (e) {
            ...
            if (hasGSAP) gsap.fromTo(kbSettingsBtn, { scale: .85 }, { scale: 1, duration: .4, ease: "elastic.out(1, 0.5)" });
        });
        ...
        kbClose.addEventListener("click", function (e) {
            ...
            e.stopPropagation();
            stopAllRepeats();
            // Exit sysmode silently (don't refocus)
            exitSystemMode({ refocus: false });
            hideKeyboard();
            setTimeout(function () {
        ...
        document.addEventListener("focusin", function (e) {
            var el = e.target;
            if (!isEditableEl(el)) return;
            if (kbWrap.contains(el)) return;   // ignores our proxy field
            if (suppressRefocus) return;

            // Focus moved to a new field while in sysmode — exit sysmode
            if (systemKbActive && el !== sysmodeTarget) {
                exitSystemMode({ refocus: false });
            }
            if (systemKbActive && el === sysmodeTarget) {
                // The target regained focus somehow — send focus back to proxy
                setTimeout(function(){
                    try { kbSysInput.focus(); } catch(e){}
                }, 0);
                return;
            }
            ...
            setTimeout(function () {
                ...
                if (rect.bottom > kbTop - 12) {
                    var scrollTarget = window.scrollY + rect.bottom - kbTop + 24;
                    try { window.scrollTo({ top: scrollTarget, behavior: "smooth" }); }
                    catch (err) { window.scrollTo(0, scrollTarget); }
                }
            }, 120);
        }, true);
        ...
        document.addEventListener("focusout", function (e) {
            var el = e.target;
            if (!isEditableEl(el)) return;
            if (kbWrap.contains(el)) return;   // our proxy field's blur is none of our business

            // If the currently focused target loses focus while in sysmode
            if (systemKbActive && el === sysmodeTarget) {
                setTimeout(function(){
                    if (!systemKbActive) return;
                    var a = document.activeElement;
                    // If focus went back to the proxy, that's fine — stay in sysmode
                    if (a === kbSysInput) return;
                    // If focus is nowhere, exit sysmode
        ...
        document.addEventListener("input", function (e) {
            var el = e.target;
            if (el === kbSysInput) return; // handled separately
            if (el === currentInput && isEditableEl(el)) {
                if (systemKbActive && el === sysmodeTarget) return;
                keyboard.setInput(getVal(el));
            }
        });
        ...
        window.addEventListener("orientationchange", function () {
            setTimeout(function () { if (settings.autoHeight) applyAutoHeight(true); }, 240);
        });
        ...
        document.addEventListener("touchmove", function (e) {
            ...
            if (e.target.closest(".simple-keyboard")) return;
